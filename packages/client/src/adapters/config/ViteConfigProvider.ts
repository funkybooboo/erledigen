...
import { ConfigProvider, ConfigError } from '@alle/shared'
...
export class ViteConfigProvider implements ConfigProvider {
  ...
  getNumber(key: string, defaultValue?: number): number {
    ...
    if (isNaN(parsed)) {
      throw new Error(
        `Configuration value for ${key} is not a valid number: ${value}`
      )
    }
  ...
  getBoolean(key: string, defaultValue?: boolean): boolean {
    ...
    throw new Error(
      `Configuration value for ${key} is not a valid boolean: ${value}`
    )
  }
